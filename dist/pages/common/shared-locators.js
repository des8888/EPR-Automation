...
export default class SharedLocator {
    ...
    async UseSearch(latestEPR, maxRetries = 5) {
    ...
    async ValidateUseSearchforNoData(latestEPR, maxRetries = 5) {
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
    ...
    async ValidateUseSearchforNoDatainDoneTab(latestEPR, maxRetries = 5) {
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            console.log(`Attempt ${attempt} to validate No Data message.`);
            // Initial search
